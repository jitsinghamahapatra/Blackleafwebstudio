...
class App {
  ...
  onTouchUp(e) {
    ...
    const distX = Math.abs(clientX - this.touchStartX);
    const distY = Math.abs(clientY - this.touchStartY);
  ...
  handleClick(clientX, clientY) {
    ...
    const x = (px / this.screen.width - 0.5) * this.viewport.width;
    const y = -(py / this.screen.height - 0.5) * this.viewport.height;
    ...
    for (let i = 0; i < this.medias.length; i++) {
      ...
      if (Math.abs(lx) <= halfW && Math.abs(ly) <= halfH) {
        const item = this.mediasImages[media.index];
        if (item && item.link) {
